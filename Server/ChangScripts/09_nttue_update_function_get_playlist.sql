drop function  public.func_get_playlist_by_id(p_playlist_id uuid);

CREATE OR REPLACE FUNCTION public.func_get_playlist_by_id(p_playlist_id uuid)
 RETURNS TABLE(playlist_id uuid, user_id uuid, username text, playlist_name text, playback_mode integer, is_public boolean, playlist_image text, song_id uuid, song_name text, singer_name text, thumnail text, number_of_listens integer, location text, link_song text)
 LANGUAGE plpgsql
AS $function$
	begin
